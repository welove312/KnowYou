...
public class AppLoaderBuilder {
    private File imageLoaderDir = null;// 应用图片缓存目录
    private static String imageCacheDir = "image_cache";

    private static AppLoaderBuilder instance;
    private AppLoaderBuilder() {
    }

    public static AppLoaderBuilder getInstance() {
        if (instance == null)
            instance = new AppLoaderBuilder();
        return instance;
    }
    ...
    private void initImageLoader(Context mContext){
        imageLoaderDir = FileUtils.getDiskCacheFile(mContext, imageCacheDir);
        ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(mContext)
                ...
                // 缓存的文件数量
                .discCache(new UnlimitedDiscCache(imageLoaderDir))
                // 自定义缓存路径
