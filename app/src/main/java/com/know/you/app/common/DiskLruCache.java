...
import com.know.you.app.BuildConfig;
...
public class DiskLruCache {
	...
	public static void clearCache(Context context, String uniqueName, FilenameFilter filter) {
		File cacheDir = getDiskCacheDir(context, uniqueName);
		clearCache(cacheDir, filter);
	}
	...
	private static void clearCache(File cacheDir, FilenameFilter filter) {
		final File[] files = cacheDir.listFiles(filter);
		for (int i = 0; i < files.length; i++) {
			files[i].delete();
		}
	}
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		final String cachePath = Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED || !FileUtils.isExternalStorageRemovable() ? FileUtils.getExternalCacheDir(context).getPath() : context
				.getCacheDir().getPath();
	...
	public static String createFilePath(File cacheDir, String key, String prefix) {
		try {
			// Use URLEncoder to ensure we have a valid filename, a tad hacky
			// but it will do for
			// this example
			return cacheDir.getAbsolutePath() + File.separator + prefix + URLEncoder.encode(key.replace("*", ""), "UTF-8");
		} catch (final UnsupportedEncodingException e) {
			Log.e(TAG, "createFilePath - " + e);
		}
	...
	public String createFilePath(String key, String prefix) {
		return createFilePath(mCacheDir, key, prefix);
	}
	...
	private boolean writeBitmapToFile(Bitmap bitmap, String file) throws IOException, FileNotFoundException {
		...
		try {
			out = new BufferedOutputStream(new FileOutputStream(file), FileUtils.IO_BUFFER_SIZE);
